...
echo "---"
export MASTER_ID=$(sudo docker run -p 29015:29015 -d mies/rethink rethinkdb --bind all)
...
echo $MASTER_PORT
export SLAVE_FIRST_ID=$(sudo docker run -p 29015 -d mies/rethink rethinkdb --join $MASTER_IP:$MASTER_PORT --bind all)
echo $SLAVE_FIRST_ID
export SLAVE_FIRST_PORT=$(sudo docker port $SLAVE_FIRST_ID 29015)
echo $SLAVE_FIRST_PORT
SLAVE_SECOND_ID=$(sudo docker run -d mies/rethink rethinkdb --join $MASTER_IP:$MASTER_PORT --bind all)
